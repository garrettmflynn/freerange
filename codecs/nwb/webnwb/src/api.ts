...
//   NWBFile?: ArbitraryObject;
  ...
  _generate(spec: any = this._specification, key?:string) {
    ...
    namespace.namespaces.forEach((namespace: any) => {
        ...
        if (namespace.name !== 'core' && this.debug) console.log(`Loading ${namespace.name} extension.`)
      ...
      namespace.schema.forEach((schema: any) => {
        ...
        if (this.debug) console.log(`WebNWB API: Generated ${namespace.name} in ${tock - tick} ms`)
    })
